...
3. **문구까지 통생성이 기본** — 배경·인물·텍스트를 전부 포함한 완성 썸네일을
   gpt_image_2 원샷으로 생성한다 (Higgsfield 경유). 프롬프트에 문구를 따옴표로
   넣고 "Render this text EXACTLY" 를 지시; 생성 후 **글자 단위 READ 검증
   필수** — 오탈자 난 변형은 같은 모델로 재생성. HTML/PIL 오버레이는 통생성이
   반복 실패할 때만 쓰는 폴백 (유저 보고 후).
...
2. **변형 축 설계 (4개 기본)**: ① 인물 리액션 + 큰 문구 ② 결과물/비포애프터
   ...
   아니라 같은 카드 4장이다.
3. **통생성** (모델 정책대로): 변형별로 구도 + 문구("...") + "Render this text
   EXACTLY, thick bold sans-serif [+ outline]" + 얼굴 레퍼런스(`--image` 실제
   사진)를 한 프롬프트에 넣어 gpt_image_2 원샷. 한국어 문구는 의미 단위
   줄나눔을 프롬프트에 명시.
